...
public class Bank extends ItemContainer {
	...
	public Bank refreshItems() {
		...
		if (getPlayer().isSearchingBank()) {
			...
			getPlayer().getPacketSender().sendString(5383, "Bank of Elvarg").sendConfig(117, 0);
		}
	...
	public static void withdraw(Player player, int item, int slot, int amount, int fromBankTab) {
		...
		if (player.getStatus() == PlayerStatus.BANKING && player.getInterfaceId() == 5292) {
			...
			// Check if we're withdrawing the item from the proper tab, but only
			// if we aren't bank searching
			...
			if (player.isSearchingBank()) {
				...
				// Withdrawing an item which belongs in another tab from the
				// main tab
				...
				// Update all tabs if we removed an item from the first item
				// slot
				if (slot == 0) {
					Bank.reconfigureTabs(player);
				}
				...
				for (int bankId = 0; bankId < TOTAL_BANK_TABS; bankId++) {
					if (button == tab_select_start + (bankId * 4)) {
						...
						if (action == 1) {
							...
							for (Item item : items) {
								player.getBank(bankId).switchItem(player.getBank(0), item.copy(),
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if (to instanceof Inventory) {
			/*
			 * boolean withdrawAsNote = getPlayer().withdrawAsNote() && def !=
			 * null && def.isNoted() && item.getDefinition() != null &&
			 * def.getName().equalsIgnoreCase(item.getDefinition().getName()) &&
			 * !def.getName().contains("Torva") &&
			 * !def.getName().contains("Virtus") &&
			 * !def.getName().contains("Pernix") &&
			 * !def.getName().contains("Torva"); int checkId = withdrawAsNote ?
			 * item.getId() + 1 : item.getId(); if(to.getAmount(checkId) +
			 * item.getAmount() > Integer.MAX_VALUE || to.getAmount(checkId) +
			 * item.getAmount() <= 0) { getPlayer().getPacketSender().
	...
	public static void depositItems(Player player, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item item : from.getValidItems()) {
			from.switchItem(player.getBank(Bank.getTabForItem(player, item.getId())), item.copy(),
